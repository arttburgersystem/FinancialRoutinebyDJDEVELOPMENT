  ...
  var rows=sorted.map(function(f){
    ...
    return el('tr',{
      ...
      el('td',{style:{padding:'8px 10px',textAlign:'right'}},[
        el('div',{style:{display:'flex',gap:'6px',justifyContent:'flex-end',alignItems:'center'}},[
          (function(){
            var num = (f.celular||f.telefone||'').replace(/\D/g,'');
            if(!num) return null;
            if(num.length<=11) num='55'+num;
            var waUrl='https://wa.me/'+num;
            var waBtn=el('a',{
              href:waUrl,target:'_blank',rel:'noopener',
              title:'Abrir WhatsApp — '+(f.celular||f.telefone),
              style:{
                display:'inline-flex',alignItems:'center',justifyContent:'center',
                width:'30px',height:'30px',borderRadius:'6px',
                background:'rgba(37,211,102,.15)',border:'1px solid rgba(37,211,102,.4)',
                textDecoration:'none',fontSize:'16px',flexShrink:'0',cursor:'pointer',
                transition:'background .15s',
              },
            },'');
            // SVG WhatsApp oficial
            var svg=document.createElementNS('http://www.w3.org/2000/svg','svg');
            svg.setAttribute('viewBox','0 0 24 24');svg.setAttribute('width','16');svg.setAttribute('height','16');svg.setAttribute('fill','#25d366');
            svg.innerHTML='<path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.125.558 4.115 1.531 5.842L.057 23.856a.5.5 0 0 0 .614.603l6.094-1.598A11.944 11.944 0 0 0 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818a9.807 9.807 0 0 1-5.032-1.389l-.36-.214-3.742.981.997-3.647-.235-.374A9.818 9.818 0 0 1 2.182 12C2.182 6.57 6.57 2.182 12 2.182S21.818 6.57 21.818 12 17.43 21.818 12 21.818z"/>';
            waBtn.appendChild(svg);
            waBtn.onmouseenter=function(){waBtn.style.background='rgba(37,211,102,.28)';};
            waBtn.onmouseleave=function(){waBtn.style.background='rgba(37,211,102,.15)';};
            return waBtn;
          })(),
